...
Create a tar archive containing all needed resources: +
`$ make dist`
...
If you use Debian, you might create a simple `deb` package using using
https://github.com/jordansissel/fpm[fpm]:

`$ make dist-deb`

Install the package and run Monsti with `monsti-daemon /etc/monsti`.
...
WARNING: Resizing is currently broken.
